...
public class NamedConstructorFunction<T> implements Function<Map<String, ?>, T> {
    ...
    public T apply(@Nullable Map<String, ?> input) {
        ...
        NamedConstructor<T> nc = findNamedConstructor(input.keySet());
        return invokeConstructor(nc.getConstructor(), input.values());
    }
    ...
    private NamedConstructor<T> findNamedConstructor(Collection<String> names) {
        ...
        throw new IllegalArgumentException("Cannot find constructor for names: '" + names + "'");
    }
